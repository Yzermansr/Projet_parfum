...
def plot_ellipse(ax, center, axes, lengths, color='r'):
    """
    Trace une ellipse centrée en `center`, avec `axes` comme directions principales et `lengths` comme longueurs.
    """
    theta = np.linspace(0, 2 * np.pi, 100)
    ellipse = np.array([lengths[0] * np.cos(theta), lengths[1] * np.sin(theta)])
    rotation_matrix = np.column_stack(axes)
    ellipse_rotated = rotation_matrix @ ellipse + center[:, None]
    ax.plot(ellipse_rotated[0], ellipse_rotated[1], color=color, linewidth=2)

def plot_polyhedron_with_ellipse(A, b):
    """
    Affiche le polyèdre défini par Ax <= b et l'ellipse autour.
    """
    fig, ax = plt.subplots(figsize=(8, 8))

    # Calcul des axes principaux
    center, axes, variances = compute_principal_axes(A, b)
    lengths = np.sqrt(variances) * 2  # Facteur d'échelle pour ajuster la taille

    # Tracer le polyèdre
    from projet_parfum.polyhedra_plot import plot_polyhedron_2d
    plot_polyhedron_2d(A, b, ax=ax, xlim=(-2, 2), ylim=(-2, 2), color='lightgray')

    # Tracer l'ellipse
    plot_ellipse(ax, center, axes, lengths, color='r')

    ax.scatter(*center, color='b', label='Centroïde')
    ax.legend()
    plt.show()
